        ...
        for dep in depot_numbers[:5]:
            try:
                ...
                if e.response is not None and e.response.status_code == 401:
                    # пробуем автологин и повтор
                    new_path = _try_autologin(update.effective_user.id)
                    if new_path:
                        await update.message.reply_text("🔄 Сессия UMP обновлена, повторяю запрос...")
                        token_path = new_path
                        try:
                            result = get_position_and_check(dep, token_path=token_path)
                            sample_results.append(result)
...
def _reset_auth_flow(user_id: int) -> None:
    ...
    # сессии/токены не трогаем, они нужны для автологина
...
def _try_autologin(user_id: int) -> Optional[str]:
    ...
    try:
        ...
        log_print(f"Автологин не удался: {e}", "ERROR")
        return None
